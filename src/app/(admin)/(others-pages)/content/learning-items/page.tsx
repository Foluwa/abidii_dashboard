...
export default function LearningItemsPage() {
  ...
  const activeFilters = useMemo(() => {
    const filters = [] as { label: string; onClear: () => void }[];
    if (selectedLanguage) {
      const lang = languages.find((l) => l.id === selectedLanguage);
      filters.push({ label: `Language: ${lang?.name || selectedLanguage}`, onClear: () => { setSelectedLanguage(''); setCurrentPage(1); } });
    }
    if (searchQuery) filters.push({ label: `Search: "${searchQuery}"`, onClear: () => { setSearchQuery(''); setCurrentPage(1); } });
    if (!isGamesRoute && filterType) {
      const typeLabel = filterType === 'game' ? 'Games' : 'Lessons';
      filters.push({ label: `Type: ${typeLabel}`, onClear: () => { setFilterType(''); setCurrentPage(1); } });
    }
    if (filterActive) {
      const statusLabel = filterActive === 'true' ? 'Active' : 'Inactive';
      filters.push({ label: `Status: ${statusLabel}`, onClear: () => { setFilterActive(''); setCurrentPage(1); } });
    }
    return filters;
  }, [selectedLanguage, languages, searchQuery, filterType, filterActive, isGamesRoute]);
  ...
  return (
    <div className="space-y-6">
      <ContentPageHeader
        title={isGamesRoute ? "Games Management" : "Learning Items"}
        subtitle={`Manage ${isGamesRoute ? 'games' : 'learning items'} for each language`}
        onAdd={handleCreate}
        addLabel="Add"
      />
